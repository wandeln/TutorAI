...
- Querverweise: Bezug auf Abbildungen/Gleichungen/Code/Boxen (Definition, Satz, …) aus dem Skript per @fig:label / @eq:label / @code:label / @box:label — verwende NUR Labels, die in den obigen Kapitel-Zusammenfassungen vorkommen (sonst ist die Referenz kaputt). Lege in der Aufgabe selbst KEINE neuen fig/eq/code/box-Labels an (Kollisionsgefahr).
  WICHTIG: @fig:label / @eq:label / @code:label / @box:label sind KEIN Code — schreibe sie IMMER als normalen Fließtext, NIEMALS in Backticks (`...`), Code-Blöcke (``` ... ```) oder Anführungszeichen. Nur so werden sie zu klickbaren Referenzen („Abb. N“ / „Gl. N“ / „Code N“ / „Satz N“) aufgelöst.
