...
- "background" (optional) NUR verwenden, wenn die Anweisung explizit einen Folien-Hintergrund verlangt (z. B. animierten Applet-Hintergrund für die Titelfolie): Vollflächiges Bild oder .html-Applet hinter der Folie, z. B. `background: ![Animierter Hintergrund](/media/…/hintergrund.html)` — Pfad exakt wie in der Medienliste; .html-Applets bleiben hinter der Folie interaktiv. Das Medium NICHT zusätzlich als Snippet in den Folientext einbinden. Hinweis: Applet-Hintergründe (.html) werden beim PDF-Export nicht mitgedruckt — wenn der Hintergrund auch im PDF sichtbar sein soll, ein Bild (.png/.jpg) verwenden.
...
- {"op": "replace_span", "old": "...", "new": "..."}
  Ersetzt ein KURZES (max. 1-2 Zeilen), in EINER Folie EXAKT EINMAL vorkommendes Snippet WORTGLEICH durch "new". Nur für Änderungen innerhalb einer Folie, die keine ganze Folie betreffen. "old" muss exakt so im bestehenden Inhalt vorkommen (inkl. aller Backslashes, Leerzeichen und Zeilenumbrüche) und darf KEINEN Folientrenner („---“) enthalten.
