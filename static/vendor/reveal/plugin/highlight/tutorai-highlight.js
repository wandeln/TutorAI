 ...
 * + eingebettetes highlightjs-line-numbers.js 2.8.0 (MIT,
 * https://github.com/wcoder/highlightjs-line-numbers.js).
 *
 * Abweichung vom Original: das offizielle Plugin ist ESM
 * (`import hljs from 'highlight.js'` → eigenes ~900-KB-Bundle) und damit
 * ohne Bundler nicht direkt im Browser lauffähig. Dieses File nutzt
 * stattdessen das GLOBAL geladene highlight.js 11.x (base.html, Theme
 * atom-one-dark) — identisches Verhalten, kein doppeltes Bundle,
 * kein Build-Step.
 *
 * Funktionen (offizielles Verhalten):
 * - Syntax-Highlighting aller <pre><code>-Blöcke (language-<lang>-Klasse)
 * - Zeilennummern: <code data-line-numbers>
 * - Schrittweises Per-Line-Reveal: data-line-numbers="1,3-5|2,4" →
 *   pro "|" ein Fragment-Clone des Blocks
 * - Zeilen-Highlight (.highlight-line; die übrigen Zeilen werden per
 *   reveal.css gedimmt)
...
const TutoraiHighlight = {
	...
	init: function( reveal ) {
		...
		if( !hljs ) {
			console.error( 'RevealHighlight: highlight.js fehlt (window.hljs undefined)' );
			return;
		}
	...
	highlightBlock: function( block ) {
		...
		if( block.hasAttribute( 'data-line-numbers' ) ) {
			if( typeof hljs.lineNumbersBlock !== 'function' ) {
				console.error( 'RevealHighlight: hljs.lineNumbersBlock fehlt (line-numbers nicht geladen)' );
				return;
			}
	...
	scrollHighlightedLineIntoView: function( block, scrollState, skipAnimation ) {
		...
		// Remember the current code block so that we can match its
		// scroll position when showing/hiding fragments
	...
	/**
	 * The easing function used for scrolling.
	 */
